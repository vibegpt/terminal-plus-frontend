...
const CollectionDetailPage: React.FC = () => {
    ...
    const loadCollectionData = async () => {
      ...
      try {
        ...
        const enhancedAmenities = collectionAmenities.map((amenity: any) => ({
          ...amenity,
          image_url: amenity.logo_url || amenity.image_url || getDefaultImage(amenity),
          slug: amenity.amenity_slug || amenity.slug || amenity.id,
          rating: amenity.rating || (4.2 + Math.random() * 0.6),
          vibe_tags: Array.isArray(amenity.vibe_tags) ? amenity.vibe_tags : [],
        ...
        // Try to use navigation state as fallback
        if (navState?.preview) {
          console.log('Using preview data as fallback');
          setCollection({
            ...navState.preview,
            terminal: terminalCode,
            airport: getAirportName(terminalCode),
            description: 'Unable to load full collection details',
            itemCount: navState.preview.itemCount || 0
          });
          setError('Unable to load amenities. Please check your connection.');
        } else {
  ...
  const getDefaultImage = (amenity: Amenity): string => {
    // Generate appropriate default images based on amenity type
    const slug = amenity.slug || amenity.amenity_slug || amenity.id;
    const images: Record<string, string> = {
      'butterfly-garden-t3': 'https://images.unsplash.com/photo-1535083783855-76ae62b2914e?w=800&h=450&fit=crop',
      'the-slide-t3': 'https://images.unsplash.com/photo-1559128010-7c1ad6e1b6a5?w=800&h=450&fit=crop',
      'jewel-viewpoint-t3': 'https://images.unsplash.com/photo-1569163139394-de4798aa62b6?w=800&h=450&fit=crop',
      'movie-theatre-t3': 'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=800&h=450&fit=crop',
      'cactus-garden-t1': 'https://images.unsplash.com/photo-1459156212016-c812468e2115?w=800&h=450&fit=crop',
      'entertainment-deck-t3': 'https://images.unsplash.com/photo-1511882150382-421056c89033?w=800&h=450&fit=crop',
      'cultural-stage-t3': 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=800&h=450&fit=crop',
      'snooze-lounge-t1': 'https://images.unsplash.com/photo-1521783988139-89397d761dce?w=800&h=450&fit=crop',
      'fish-spa-t1': 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=800&h=450&fit=crop',
      'gordon-ramsay-t5': 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=800&h=450&fit=crop',
      'aw-root-beer-t3': 'https://images.unsplash.com/photo-1521017432531-fbd92d768814?w=800&h=450&fit=crop',
      'concorde-room-ba': 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=450&fit=crop',
      'sofitel-dayroom-t5': 'https://images.unsplash.com/photo-1578683010236-d716f9a3f461?w=800&h=450&fit=crop',
      'ba-heritage-t5': 'https://images.unsplash.com/photo-1565043589221-1a6fd9ae45c7?w=800&h=450&fit=crop',
      'prayer-room-t5': 'https://images.unsplash.com/photo-1569163139394-de4798aa62b6?w=800&h=450&fit=crop',
      'qantas-first-lounge-syd': 'https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=800&h=450&fit=crop',
    ...
    if (amenity.category?.toLowerCase().includes('food') || amenity.category?.toLowerCase().includes('restaurant')) {
    ...
    if (amenity.category?.toLowerCase().includes('garden') || amenity.category?.toLowerCase().includes('nature')) {
  ...
  const allVibes = Array.from(new Set(amenities.flatMap(a => a.vibe_tags || [])));

  // Filter amenities
  const filteredAmenities = filterVibe === 'all' 
    ? amenities 
    : amenities.filter(a => a.vibe_tags?.includes(filterVibe));
                      ...
                      <div className="relative aspect-video">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4">
                          <div className="flex items-center gap-2 mb-2">
                            {amenity.vibe_tags?.slice(0, 2).map(tag => (
                              <span 
                                key={tag}
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="text-center p-3 bg-gray-800 rounded-lg">
                  ...
                  <p className="font-semibold">{selectedAmenity.price_tier || selectedAmenity.price_level || 'Free'}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  onClick={() => handleSaveAmenity(selectedAmenity.id || '')}
                  className="px-6 py-3 bg-gray-800 rounded-lg hover:bg-gray-700 transition-all"
                >
                  <Heart className={`h-5 w-5 ${
                    savedAmenities.includes(selectedAmenity.id || '') ? 'text-red-500 fill-current' : ''
                  }`} />
                </button>
              </div>
            </div>
