...
const vibes = [
  ...
  { value: "Shop", label: "🛍️ Shop", glow: "vibe-glow-shop" }
];
...
export default function TransitGuide() {
  ...
  // Use the new Sydney amenities file if airport is SYD
  const amenitiesData: TerminalAmenity[] = useMemo(() => {
    // Default for other airports
    return (amenitiesData as unknown as any[]).map((amenity: any) => ({
      name: amenity.name,
