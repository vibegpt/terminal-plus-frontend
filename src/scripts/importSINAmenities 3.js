...
async function importTerminalData(terminalData, terminalCode) {
  try {
    const amenities = terminalData.amenities || terminalData;
    
    console.log(`🔄 Processing ${amenities.length} amenities for ${terminalCode}...`);
    
    const formattedAmenities = amenities.map((amenity, index) => {
      // Progress indicator
      if (index % 50 === 0) {
      ...
      return {
        ...
        vibe_tags: amenity.vibes?.join(',') || amenity.vibe_tags || '',
