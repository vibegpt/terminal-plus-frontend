...
import { useVibeContext } from '@/context/VibeContext';
...
function OpeningHours({ hours }: { hours: { days: string, status: string }[] }) {
  ...
  return (
    <div className="mt-2 flex items-start gap-2 text-slate-800 text-sm">
      <Clock className="w-4 h-4 mt-0.5 text-slate-600" aria-label="Opening hours" />
      <div>
...
export default function AmenityDetail() {
  ...
  const { selectedVibe } = useVibeContext();
  ...
  let amenity = null;
  if (slug) {
    // Try to find in recommendations mapping first
    const title = slug.split("-").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
    amenity = recommendationToAmenity[title];
    
    // If not found in recommendations, look in the amenities data
    if (!amenity) {
      // Try to find by exact slug match (now that all amenities have slugs)
      amenity = (amenitiesData as any[]).find(a => a.slug === slug);
      
      // Fallback: try to find by name (converting slug back to name)
      if (!amenity) {
        const nameFromSlug = slug.split("-").map(word => 
          word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
        ).join(" ");
        amenity = (amenitiesData as any[]).find(a => 
  ...
  let amenityVibe = selectedVibe || 'Relax';
  if (!selectedVibe && amenity && amenity.vibe_tags && amenity.vibe_tags.length > 0) {
  ...
  const filteredAmenities = amenitiesData.filter((a: any) => {
  ...
  const pageBgClass = vibeBackgrounds[safeAmenityVibe] || vibeBackgrounds['Relax'];
  const headline = vibeHeadlines[safeAmenityVibe] || vibeHeadlines['Relax'];
  const badgeColor = vibeColor[safeAmenityVibe] || vibeColor['Relax'];
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center ${pageBgClass}`}>
      <div className="rounded-3xl shadow-xl p-6 max-w-md w-full flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(2px)' }}>
        ...
        <div className="mb-2 text-center">
          <h2 className="text-xl font-bold mb-1 text-slate-800">{headline}</h2>
        </div>
        <div className="flex flex-col items-center gap-2 mb-2">
          <span className="text-3xl font-bold mb-1" style={{ color: '#000000' }}>{amenity.name}</span>
          <span className="inline-block rounded-full px-3 py-1 text-sm font-semibold text-slate-800" style={{ background: badgeColor + '33', color: '#1f2937' }}>{safeAmenityVibe}</span>
        </div>
        <div className="flex flex-col gap-1 mb-2">
          <div className="flex items-center gap-2 text-slate-800 text-sm"><span className="font-semibold">Location:</span> <span>{parseLocation(amenity.location_description).concise || amenity.location_description.replace(/After Security,? ?/i, '')}</span></div>
          {amenity.distance && <div className="flex items-center gap-2 text-slate-800 text-sm"><span className="font-semibold">Distance:</span> <span>{amenity.distance}</span></div>}
          ...
          {amenity.price_tier && <div className="flex items-center gap-2 text-slate-800 text-sm"><span className="font-semibold">Price:</span> <span>{amenity.price_tier}</span></div>}
        </div>
        ...
        <div className="bg-white/80 rounded-lg p-4 text-slate-800 text-base text-center shadow" style={{ border: `1.5px solid ${badgeColor}33` }}>
