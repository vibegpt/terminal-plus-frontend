...
// ── Flight Context Boost (Phase 3) ─────────────────────────────────

interface FlightContextForBoost {
...
interface BoostedAmenity extends AmenityForSelection {
  score: number;
  flightBoost?: number;
}

export function applyFlightContextBoost(
  scoredAmenities: BoostedAmenity[]
): BoostedAmenity[] {
  let flightContext: FlightContextForBoost | null = null;
  try {
    const stored = sessionStorage.getItem('terminal_plus_flight');
    flightContext = stored ? JSON.parse(stored) : null;
  } catch { return scoredAmenities; }

  if (!flightContext || flightContext.minutesUntilBoarding === 0) {
    return scoredAmenities;
  }

  const mins = flightContext.minutesUntilBoarding;
  const circadian = flightContext.circadianState;
  const phase = flightContext.journeyPhase;
  const flightTerminal = `SIN-${flightContext.terminal}`;

  return scoredAmenities.map(amenity => {
    let boost = 0;
    const vibes = (Array.isArray(amenity.vibe_tags)
      ? amenity.vibe_tags
      : String(amenity.vibe_tags || '').split(',')
    ).map(v => v.trim().toLowerCase());

    // TIME PRESSURE BOOSTS
    if (mins < 30) {
      if (vibes.includes('quick'))   boost += 15;
      if (vibes.includes('refuel'))  boost += 5;
      if (vibes.includes('explore')) boost -= 10;
      if (vibes.includes('shop'))    boost -= 10;
      if (vibes.includes('comfort')) boost -= 8;
    } else if (mins <= 60) {
      if (vibes.includes('quick'))   boost += 8;
      if (vibes.includes('refuel'))  boost += 5;
    } else if (mins > 120) {
      if (vibes.includes('explore')) boost += 8;
      if (vibes.includes('comfort')) boost += 6;
      if (vibes.includes('shop'))    boost += 5;
      if (vibes.includes('quick'))   boost -= 3;
    }

    // CIRCADIAN BOOSTS
    if (circadian === 'morning') {
      if (vibes.includes('refuel')) boost += 6;
      if (vibes.includes('work'))   boost += 3;
    } else if (circadian === 'afternoon') {
      if (vibes.includes('explore')) boost += 4;
      if (vibes.includes('refuel'))  boost += 3;
    } else if (circadian === 'evening') {
      if (vibes.includes('refuel')) boost += 5;
      if (vibes.includes('shop'))   boost += 4;
      if (vibes.includes('chill'))  boost += 3;
    } else if (circadian === 'night') {
      if (vibes.includes('comfort')) boost += 8;
      if (vibes.includes('chill'))   boost += 5;
      if (vibes.includes('explore')) boost -= 5;
    }

    // JOURNEY PHASE BOOSTS
    if (phase === 'transit') {
      if (vibes.includes('explore')) boost += 5;
      if (vibes.includes('comfort')) boost += 4;
    } else if (phase === 'departure') {
      if (vibes.includes('shop'))  boost += 3;
      if (vibes.includes('quick')) boost += 2;
    } else if (phase === 'arrival') {
      if (vibes.includes('quick'))  boost += 6;
      if (vibes.includes('refuel')) boost += 3;
      if (vibes.includes('shop'))   boost -= 5;
    }

    // TERMINAL PROXIMITY
    if (amenity.terminal_code === flightTerminal) {
      boost += 4;
    } else if (mins < 45) {
      boost -= 3;
    }

    return { ...amenity, score: amenity.score + boost, flightBoost: boost };
