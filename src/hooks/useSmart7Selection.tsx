...
import { Smart7Algorithm, applyFlightContextBoost } from '../utils/smart7Algorithm';
...
export const useSmart7Selection = ({
   ...
   */
  const runSelection = useCallback(async (mounted: boolean = true) => {
    ...
    try {
      ...
      // Apply flight context boost (Phase 3)
      const boostedAmenities = applyFlightContextBoost(
        results.map(r => ({ ...r.amenity, score: r.score.totalScore }))
      );
      // Re-rank results based on boosted scores
      const boostedResults = boostedAmenities.map((boosted, idx) => {
        const original = results.find(r => r.amenity.id === boosted.id);
        return original
          ? { ...original, rank: idx + 1, contextData: { ...original.contextData, flightBoost: boosted.flightBoost } }
          : results[idx];
      });

      if (mounted) {
        setSelections(boostedResults);
  ...
  const refresh = useCallback(async () => {
    ...
    await runSelection(true); // Always pass true for manual refresh
  }, [runSelection]);
