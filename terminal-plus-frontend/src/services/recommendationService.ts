...
// Update the match score calculation to include vibe_tags boost
const calculateMatchScore = (
  ...
  // Boost if the amenity has the selected vibe in its vibe_tags
  const vibeTagBoost = (amenity.vibe_tags || []).map(tag => tag.toLowerCase()).includes(context.active_vibe.toLowerCase()) ? 0.2 : 0;
...
const amenityToRecommendedAction = (
  ...
  return {
    ...
    slug: amenity.slug,
    description: amenity.location_description,
