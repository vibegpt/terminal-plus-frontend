  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      const filtered = allAmenities.filter(a => {
        // Check if it matches the specific terminal for the given airport
        const terminalMatches = a.terminal_code === `${data.origin}-${terminal}`;
        if (!terminalMatches) {
          return false;
        }
      ...
      console.log('Filtered amenities:', {
        terminalCode: `${data.origin}-${terminal}`,
